...
def compute_land_tax(year: int) -> dict[int, Decimal] | None:
    ...
    - ИТОГО для приватизированного гаража: налог на общую территорию МИНУС
      ...
      стандартной (garage.privatized_land_area - standard_garage_land_area,
      если положительно) — участок уже приватизирован и платится напрямую
      государству, а если он ещё и больше стандартного надела, разница
      вычитается из доли в общей территории (не суммируется поверх); при
      отрицательном результате начисление обнуляется, а не становится
      отрицательным

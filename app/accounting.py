...
def person_member_account_number(
    ...
    ОБЩАЯ длина номера, см. member_account_number) заполнитель в ширину
    owner_digits всё равно дописывается, только нулями, а не индексом, и
    СРАЗУ ПОСЛЕ кода типа (а не в конце, после id человека) — по прямой
    просьбе: "30095" для type_code="3", person_id=95, owner_digits=1
    (по умолчанию), не "30950" — иначе нулевой заполнитель зрительно
    сливается с самим id человека и путается с ним. Так номер по-прежнему
    не выделяется длиной среди счетов, заведённых по гаражу ("20950" у
    земельного налога и т.п.), хотя по сути это не "нулевой собственник",
    а просто заполнитель длины.
    """
    settings = settings or get_settings()
    return f"{fee_type_code}{'0' * settings.owner_digits}{_padded_digits(person_id, settings.garage_digits)}"
...
def compute_land_tax(year: int) -> dict[int, Decimal] | None:
    ...
    - ИТОГО для приватизированного гаража: (налог на общую территорию МИНУС
      стоимость превышения фактической приватизированной площади над
      стандартной, если положительно, иначе 0) * (1 + % банка) — участок
      уже приватизирован и платится напрямую государству, а если он ещё и
      больше стандартного надела, разница вычитается из доли в общей
      территории (не суммируется поверх); % банка применяется в любом
      случае — это комиссия за зачисление платежа на счёт кооператива, не
      зависит от того, приватизирован участок или нет
    ...
    for garage in garages:
        if garage.land_privatized:
            ...
            # Если фактически приватизированная площадь больше стандартной
            # (той, что для неприватизированных гаражей учитывается как
            # "площадь под гаражом") — стоимость превышения вычитается из
            # доли в налоге на общую территорию: разница уже "выкуплена"
            # собственником у кооператива при приватизации сверх нормы, не
            # повод дополнительно её ему начислять. Отрицательный итог
            # обнуляется — начисление не превращается в переплату
            # кооператива в пользу собственника.
            if garage.privatized_land_area is not None and garage.privatized_land_area > standard_area:
                excess_area = garage.privatized_land_area - standard_area
                garage_tax = max(garage_tax - excess_area * price_per_sqm, Decimal("0"))
            # % банка — комиссия за зачисление платежа на счёт кооператива,
            # применяется независимо от приватизации участка (см. докстринг).
            garage_tax = garage_tax * bank_multiplier
        else:
...
def account_digits(value: str | None) -> str:
    """
    ...
    (как на банковской выписке) — именно из-за лишних пробелов Сбербанк
    отказывался принимать QR-код («Нельзя оплатить по этому QR-коду»):
    PersonalAcc получался длиннее 20 символов и переставал быть валидным
    номером счёта. Тем же способом печатается и видимый номер Р/сч на самой
    квитанции ПД-4 (см. pd4/print.html — Jinja-глобал, зарегистрирован в
    app/__init__.py) — без пробелов, независимо от того, как он записан в
    реквизитах.
    """
...
def pd4_qr_payload(coop: Cooperative, bank_account, member_account: MemberAccount, amount: Decimal) -> str:
    ...
    fields = {
        ...
        "PersonalAcc": account_digits(bank_account.checking_account) if bank_account else "",
        "BankName": bank_account.bank_name if bank_account else "",
        "BIC": account_digits(bank_account.bik) if bank_account else "",

...
def detail(garage_id):
    ...
    # не его дело; правление видит все. Счета с нулевым балансом (пеня без
    # начислений, "telecom_disputes" и т.п., как и у полностью погашенных)
    # НЕ отфильтровываются здесь — это дело чекбокса «Актуальные» на
    # клиенте (data-zero-noncore, см. garages/detail.html и
    # models.CORE_FEE_TYPE_CODES), иначе для таких счетов чекбокс не
    # работал бы: строка просто не долетала бы до шаблона ни в каком
    # состоянии галки.
    member_accounts = (
        database.db_session.query(MemberAccount)

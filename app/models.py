...
class News(Base):
    """Новостная лента на главной странице сайта (см. app/main.py: index()).
...
# "Основные" виды взноса — по коду FeeType (см. ниже). Используется в двух
# не связанных напрямую местах, но по одной и той же причине (по прямой
# просьбе правления):
# 1. legal_docs.debt_categories() — какие виды долга по умолчанию попадают
#    в уведомление о задолженности/иск (за электричество, за
#    "telecom_disputes" и любые другие виды кооператив не судится и не
#    рассылает претензии по умолчанию — там отключение/иной механизм).
# 2. Чек-бокс «Актуальные» у таблиц лицевых счетов (persons/detail.html,
#    garages/detail.html, finance/member_accounts.html, см.
#    data-zero-noncore/initActiveAccountsToggle в base.html) — счета ЭТИХ
#    видов показываются всегда, даже с нулевым балансом (они есть у каждого
#    члена кооператива по определению); счета остальных видов (пеня,
#    "telecom_disputes", любые будущие) с нулевым балансом считаются шумом
#    и по умолчанию скрыты — именно так, а не по is_penalty, чтобы
#    правило было одно и то же и для пени, и для прочих редких видов.
CORE_FEE_TYPE_CODES = {"membership", "target", "land_tax"}

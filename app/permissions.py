...
def is_owner_or_board(garage) -> bool:
    ...
    собственником, а не только значиться в списке). Не даёт управлять самим
    списком контактов гаража (add/edit/remove_contact) — там форма выбора
    человека берёт весь реестр членов кооператива (all_persons), а он
    рядовому собственнику видеть не должен, поэтому это отдельно ограничено
    is_board() и в шаблоне, и в garages.py."""
    if is_board():
        return True

...
# (роутеры, DVR и т.п.) и архивов с такими конфигами/бэкапами, поэтому
# список дополнен текстовыми форматами конфигов и архивами. Ни один из них
# не исполняется браузером как скрипт/разметка (в отличие от .html/.svg/.js,
# которые намеренно нигде не разрешены — см. комментарий к
# DEFAULT_ALLOWED_EXT выше), так что риск stored XSS тот же. os.path.splitext
# в save_upload() берёт только ПОСЛЕДНЕЕ расширение — для "архив.tar.gz"
# это ".gz", отдельно перечислять ".tar.gz" не нужно.
WIKI_ATTACHMENT_ALLOWED_EXT = DEFAULT_ALLOWED_EXT | {
    ".conf", ".cfg", ".ini", ".json", ".xml", ".yaml", ".yml", ".ovpn",

...
России, если в БД ещё вообще ничего нет), по сегодня. Сама автоматическая
пеня в приложении и так пересчитывается «на лету» при каждом открытии
дашборда/страницы «Пеня» (см. app/penalty.py, app/main.py) — этот скрипт
нужен только чтобы ставка была свежей ещё ДО того, как кто-то из правления
откроет приложение (например, чтобы пеня не «запаздывала» по выходным/
отпуску правления, когда в систему никто не заходит днями).
